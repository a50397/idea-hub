...
- Prod redeploy needed to pick up: i18n fix (translations broken in deployed images since March), hardened images, Mongo auth (fresh volume); deployment env must now also set `MAIL_SETTINGS_KEY` (prod compose fail-fast).
