...
- Odchádzajúce HTTPS na hostiteľa Jira Cloud (napr. `https://firma.atlassian.net`)
  — len ak sa bude používať voliteľná Jira integrácia (zapína ju admin v UI na
  stránke **Nastavenia Jiry**). Bez tohto prístupu aplikácia funguje normálne,
  iba sa nikdy nepodarí vytvoriť ani synchronizovať úlohu v Jire (Test pripojenia
  aj pravidelný poller zlyhajú).
...
- **Upgrade**: nové tagy obrazov → `pull` (alebo `load`) → `up -d --no-build`
  ...
  realizácie. Vyžaduje aj odchádzajúce HTTPS na hostiteľa Jira Cloud (pozri §2).
